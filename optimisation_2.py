...
def evaluate_sphere_containment_ratio(center, radius, origin, R_matrix_inv, r_min, r_max, theta_max_deg, z_max):
    ...
    Returns the exact ratio of sample points trapped inside the region window.
    """
...
class MarchingAnnularManager:
    def __init__(self, sphere_radius, box_dim):
        ...
        self.generate_random_valid_sector()
        self.run_containment_analysis(self.radius, label="Initial State")

    def generate_random_valid_sector(self):
        print("Searching for a valid random orientation completely inside the box...")
        attempts = 0
        while True:
            attempts += 1
            ...
            if is_sector_completely_inside_box(rand_origin, R, self.s_rmin, self.s_rmax,
                ...
                self.origin = rand_origin
                self.R_matrix = R
                print(f"Valid configuration found on attempt {attempts}!")
                break
    ...
    def run_containment_analysis(self, target_radius, label="Current State"):
        ...
        count_0 = 0  # > 0% Enclosed
        count_25 = 0  # > 25% Enclosed
        count_50 = 0  # > 50% Enclosed
        count_75 = 0  # > 75% Enclosed
        ...
        # Retain initial highlighted list tracking sequence for rendering filters
    ...
    def expi(self, expansion_offset):
        ...
        # Recalculate distribution breakdown metrics post expansion transformation
...
if __name__ == "__main__":
    ...
    manager = MarchingAnnularManager(sphere_radius=BASE_RADIUS, box_dim=BOX_SIZE)
